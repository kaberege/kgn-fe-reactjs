...
export default function QuizDetails() {
  ...
  useEffect(() => {
    const historyDetails =
      JSON.parse(localStorage.getItem("historyquiz")) || []; // Fetching quiz history from local storage
    const currentDetails =
      historyDetails.find((item) => item.id === parseInt(id)) || {}; // Filtering the matching quiz
              ...
              {details.details.map((item, index) => (
                ...
                >
                  <p className="font-medium">
                    {index + 1}. {item.question && he.decode(item.question)}
                  </p>
                  <p className="mt-1">
                    <strong>Your Answer:</strong>{" "}
                    {item.choosed && he.decode(item.choosed)}
                  ...
                  <ul className="list-disc list-inside">
                    {item.options.map((option, idx) => (
                      <li
                        ...
                        style={{
                          color:
                            option === item.correct ? "#68d391" : "#e2e8f0", // text-green-300 or text-gray-300
                        }}
                        className="p-1"
                      >
                        {he.decode(option)}
