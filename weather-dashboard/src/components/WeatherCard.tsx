...
export default function WeatherCard() {
  ...
  const getBackground = (weather: string) => {
    switch (weather) {
      case "Clear":
        return {
          setSuggestColor:
            "bg-blue-700 text-white hover:bg-blue-800 dark:bg-blue-500 dark:text-white dark:hover:bg-blue-600",
          container: "bg-blue-100 dark:bg-blue-900", // Clear weather: light blue container, dark blue button
        };
      case "Rain":
        return {
          setSuggestColor:
            "bg-blue-800 text-white hover:bg-blue-900 dark:bg-blue-600 dark:text-white dark:hover:bg-blue-700",
          container: "bg-blue-300 dark:bg-blue-800", // Rain: darker button for contrast
        };
      case "Clouds":
        return {
          setSuggestColor:
            "bg-gray-800 text-white hover:bg-gray-900 dark:bg-gray-600 dark:text-white dark:hover:bg-gray-900",
      ...
      case "Thunderstorm":
        return {
          setSuggestColor:
            "bg-yellow-600 text-black hover:bg-yellow-700 dark:bg-yellow-500 dark:text-black dark:hover:bg-yellow-600",
          container: "bg-yellow-300 dark:bg-yellow-700", // Thunderstorm: yellow for contrast
        };
      case "Drizzle":
        return {
          setSuggestColor:
            "bg-blue-600 text-white hover:bg-blue-700 dark:bg-blue-400 dark:text-white dark:hover:bg-blue-500",
          container: "bg-blue-200 dark:bg-blue-600", // Drizzle: lighter blue container, dark button
        };
      case "Mist":
        return {
          setSuggestColor:
            "bg-gray-700 text-white hover:bg-gray-800 dark:bg-gray-400 dark:text-white dark:hover:bg-gray-700",
          container: "bg-gray-200 dark:bg-gray-600", // Mist: soft gray container, dark button
        };
      default:
        return {
          setSuggestColor:
            "bg-gray-800 text-white hover:bg-gray-900 dark:bg-gray-600 dark:text-white dark:hover:bg-gray-700",
  ...
  function weatherConditions(condition: string) {
    ...
    switch (condition) {
      ...
      case "Clouds":
        iconImage = (
          <BsCloudFill size={60} className="text-gray-700 dark:text-gray-300" />
        );
        break;
  ...
  async function fetchLatLongData(lat: number, lon: number) {
    ...
    const url = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&APPID=${VITE_API_KEY}&units=metric`;
    setLoad("Loading...");
    try {
  ...
  return (
    <div
      className={`${
        weather !== null &&
        Object.keys(weather).length > 2 &&
        getBackground(weather?.weather[0].main || "Clear").container
      }
    ...
    >
      ...
      {/* Error Message */}
      {serchError && (
        <p className="text-red-500 text-center mt-2">{serchError}</p>
      )}

      {load ? (
        <p className="text-center mt-3 text-gray-500">{load}</p>
      ) : weather !== null && Object.keys(weather).length > 2 ? (
        <div className="flex flex-col gap-4">
              ...
              >
                {suggestions.map((suggestion, index) => (
                  <button
                    ...
                    className={`${
                      getBackground(weather?.weather[0].main || "Clear")
                        .setSuggestColor
          ...
          <div className="flex flex-col items-center mb-3">
            ...
            <p className="text-xl max-sm:text-lg mt-4 font-semibold">
          ...
          <div className="flex justify-center items-center relative">
            ...
            <p
              className={`text-center text-stone-700 text-xs font-semibold p-2 absolute top-[120%] w-36 bg-white rounded-lg shadow-lg transition-opacity duration-300 ${
                info && "hidden"
              }`}
            >
