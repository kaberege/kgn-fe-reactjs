...
const TripForm = () => {
  ...
  const fetchCoordinates = async (location: string) => {
    ...
    try {
      const API_URL = import.meta.env.VITE_OPEN_CAGE_API_KEY; // The OpenCage
      const response = await axios.get(
        `https://api.opencagedata.com/geocode/v1/json?q=${location}&key=${API_URL}`
      );
  ...
  const handleSubmit = async (
    ...
    if (isNaN(cycleUsed) || cycleUsed < 0) {
      setErrorMessage(
        "Please enter a valid non-negative number for the hours."
      );
      return;
    }
    ...
    if (cycleUsed > 8.75) {
      setErrorMessage(
        "Driving hours cannot exceed the allowed maximum of 8.75 hrs/day"
      );
      return;
    }
