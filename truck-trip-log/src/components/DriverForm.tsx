...
const DriverForm = ({
  ...
  const handleDutyTimeChange = (
    timePeriod: keyof DutyHours,
    type: "driving" | "offDuty" | "onDuty" | "sleeperBerth",
    input: number
  ) => {
    ...
    setDutyTimes((prev) => {
      ...
      const totalHours = [
        ...
        acc += Object.values(updatedTimes[key + "Hours"]).reduce(
          (sum, hours) => sum + hours,
          0
        );
        return acc;
      ...
      const totalMorningHrs: number =
        updatedTimes.drivingHours["0:00-11:59"] +
        updatedTimes.offDutyHours["0:00-11:59"] +
        updatedTimes.onDutyHours["0:00-11:59"] +
        updatedTimes.sleeperBerthHours["0:00-11:59"];
      const totalAfterNoonHrs: number =
        updatedTimes.drivingHours["12:00-17:59"] +
        updatedTimes.offDutyHours["12:00-17:59"] +
        updatedTimes.onDutyHours["12:00-17:59"] +
        updatedTimes.sleeperBerthHours["12:00-17:59"];
      const totalNightHrs: number =
        updatedTimes.drivingHours["18:00-23:59"] +
        updatedTimes.offDutyHours["18:00-23:59"] +
        updatedTimes.onDutyHours["18:00-23:59"] +
        updatedTimes.sleeperBerthHours["18:00-23:59"];

      // Validate total driving hours do not exceed Current Cycle Used (hrs)
      const totalDrivingHours = Object.values(updatedTimes.drivingHours).reduce(
        (acc, hours) => acc + hours,
        0
      );
      ...
      if (totalDrivingHours > currentCycleUsed) {
        setError(
          `Driving hours cannot exceed the Current Cycle Used (hrs) of ${currentCycleUsed}.`
        );
        return prev; // Prevent state update if the total driving hrs exceed Current Cycle Used (hrs)
      } else if (totalHours > 24) {
        setError(
          "Total hours (driving, off-duty, on-duty, and sleeper-berth) cannot exceed 24 hours for any period."
        );
        return prev; // Prevent state update if the total exceeds 24
      } else if (totalMorningHrs > 12) {
        setError(
          "Total moring hours[0:00-11:59] (driving, off-duty, on-duty, and sleeper-berth) cannot exceed 12 hours."
        );
        return prev; // Prevent state update if the total morning hours exceeds 12
      } else if (totalAfterNoonHrs > 6) {
        setError(
          "Total afternoon hours[12:00-17:59] (driving, off-duty, on-duty, and sleeper-berth) cannot exceed 6 hours."
        );
        return prev; // Prevent state update if the total afternoon hours exceeds 6
      } else if (totalNightHrs > 6) {
        setError(
          "Total night hours[18:00-23:59] (driving, off-duty, on-duty, and sleeper-berth) cannot exceed 6 hours."
        );
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const totalDrivingHours = Object.values(dutyTimes.drivingHours).reduce(
      (acc, hours) => acc + hours,
      0
    );
    const totalOffDutyHours = Object.values(dutyTimes.offDutyHours).reduce(
      (acc, hours) => acc + hours,
      0
    );
    const totalOnDutyHours = Object.values(dutyTimes.onDutyHours).reduce(
      (acc, hours) => acc + hours,
      0
    );
    const totalSleeperBerthHours = Object.values(
      dutyTimes.sleeperBerthHours
    ...
    const overallDutyHours: number =
      totalDrivingHours +
      totalOffDutyHours +
      totalOnDutyHours +
      totalSleeperBerthHours;
    ...
    if (overallDutyHours < 24 || overallDutyHours > 24) {
      ...
      setError(
        "You can not drive without moving. Check total miles and driving hours:"
      );
    } else if (driverDetails.miles <= 0 && totalDrivingHours > 0) {
      setError(
        "You can not drive without moving. Check total miles and driving hours"
      ...
      setError(
        `Adjust your hours! Your Current Cycle Used (hrs): ${currentCycleUsed}, but you are sending: ${totalDrivingHours}`
      );
    } else {
      sendDataToBackend();
    }
  ...
  const sendDataToBackend = async () => {
    ...
    const currentCycleUsed = driverTripData.currentCycleUsed
      ? parseFloat(driverTripData.currentCycleUsed)
      : 8.75; // Current Cycle Used (hrs)
  ...
  return (
    <div className="pt-2">
      ...
      <div className="max-w-2xl mx-auto bg-stone-100 border border-stone-600 rounded-lg p-1 mt-2 mb-7 sm:p-6 shadow-lg dark:bg-gray-800">
        <h1 className="text-[19px] sm:text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4 text-center">
          Driver Information
        </h1>
            ...
            {["0:00-11:59", "12:00-17:59", "18:00-23:59"].map((timePeriod) => (
              <div key={timePeriod} className="space-y-1 mt-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div>
                    <label
                      htmlFor={`sleeperBerth-${timePeriod}`}
                      className="text-sm text-gray-700 dark:text-gray-300"
                    >
                      Sleeper Berth Hours
                    </label>
