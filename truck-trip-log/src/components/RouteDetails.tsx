...
const RouteDetails = ({
  ...
  const formatDistance = (distance: number | null) => {
    ...
    return distance < 1000
      ? `${distance.toFixed(2)} meter${distance > 1 ? "s" : ""}`
  ...
  return (
    <>
      ...
      <div
        className={`absolute top-8 right-1 sm:right-2 z-[1000] p-1 sm:p-2 bg-slate-100 dark:bg-gray-800 rounded-lg shadow-lg transition-all ${
          isExpanded ? "h-auto" : "h-7 sm:h-9 overflow-hidden"
        }`}
      >
        ...
        <div
          className={`text-xs sm:text-xs font-medium text-gray-700 dark:text-gray-300 transition-all ${
        ...
        <div
          className={`transition-all flex flex-col justify-center ${
            isExpanded ? "opacity-100" : "opacity-0 pointer-events-none"
          }`}
        >
          {error && (
            <button
              onClick={fetchRoute}
              className="mt-2 text-xs text-blue-700 hover:text-blue-400 underline transition-colors duration-300 cursor-pointer"
            >
              Retry
            </button>
          )}
